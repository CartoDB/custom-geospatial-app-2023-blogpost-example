...
const countAccidentsPerState = `

SELECT
    polygonsb.*,
    COALESCE(__count.count, 0) AS count
  FROM \`carto-demo-data.demo_tables.usa_states_boundaries\` polygonsb
  LEFT JOIN
    (
      SELECT
        polygons.code_hasc poligonid,
        count(*) count
      FROM
        \`carto-demo-data.demo_tables.usa_states_boundaries\` as polygons,
        \`carto-demo-data.demo_tables.riskanalysis_railroad_accidents\` as points
      WHERE
          ST_INTERSECTS(
            polygons.geom,
            points.geom
          ) AND
          points.year = @selectedYear
      GROUP BY
          polygons.code_hasc
    ) __count
  ON __count.poligonid = polygonsb.code_hasc;
